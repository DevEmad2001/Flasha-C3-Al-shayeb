...
export const Route = createFileRoute("/monthly")({ component: MonthlyPage });
...
function MonthlyPage() {
  ...
  const { data } = useQuery({
    queryKey: ["monthly", owner, start, end, ownerProjectIds.join(",")],
  ...
  const monthlyBreakdown = useMemo(() => {
    ...
    while (cy < ey || (cy === ey && cm <= em)) {
      ...
      const data = map.get(key) ?? { in: 0, out: 0 };
      months.push({ label: `${monthNames[cm - 1]} ${cy}`, in: data.in, out: data.out, net: data.in - data.out });
      cm++;
      if (cm > 12) { cm = 1; cy++; }
    }
    return months;
  ...
  const handleExport = () => {
    ...
    const fileName = `جرد_شهري_${owner === "__all__" ? "الكل" : owner}_${start}_${end}`;
  ...
  return (
    <div className="p-3 sm:p-6 max-w-7xl mx-auto w-full min-w-0">
      <PageHeader
        title="الجرد الشهري"
        subtitle="عرض كل الحركات المالية لمالك معين خلال فترة محددة"
